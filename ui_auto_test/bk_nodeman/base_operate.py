...
import logging
import os
from bk_nodeman.base_info import BaseInfomation
...
class BaseOperate():
    ...
    def read_test_url(self):
        ''' 获取配置文件中的URL'''
        url = self.read_data(key=settings.NODEMAN_URL)
        if not url:
            logging.info('无法获取对应测试环境URL,测试结束')
            return False
        elif 'ee207' in url:
            settings.ENVIRONMENT = False
            settings.NODEMAN_URL = url
            logging.info('测试地址获取成功')
            logging.info('当前测试地址为{}'.format(settings.NODEMAN_URL))
            return url
        else:
