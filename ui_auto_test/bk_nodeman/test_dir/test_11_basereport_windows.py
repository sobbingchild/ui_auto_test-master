...
class WindowsBasereport(seldom.TestCase):
    def test_01_stop_basereport(self):
        """执行停止basereport插件（windows主机）"""
    ...
    def test_02_start_basereport(self):
        """执行启动basereport插件（windows主机），断言停止启动是否成功"""
        # 进入插件管理
        test_01_login.Login.test_login(self)
        test_01_login.Login.click_plug_in_management(self)
        sleep(1)
