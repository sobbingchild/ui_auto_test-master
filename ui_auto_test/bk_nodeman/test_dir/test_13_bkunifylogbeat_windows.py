...
class WindowsBkunifylogbeat(seldom.TestCase):
    def test_01_stop_bkunifylogbeat(self):
        """执行停止bkunifylogbeat插件（windows主机）"""
    ...
    def test_02_start_bkunifylogbeat(self):
        """执行启动bkunifylogbeat插件（windows主机），断言停止启动是否成功"""
        # 进入插件管理
        test_01_login.Login.test_login(self)
        test_01_login.Login.click_plug_in_management(self)
        # 点击搜索框，选择操作系统为windows
